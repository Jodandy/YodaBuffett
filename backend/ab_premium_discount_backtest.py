#!/usr/bin/env python3
"""
A/B Share Premium-Discount Arbitrage Strategy

Only buys the historically premium share when it trades at a discount
to the historically cheap share. Much simpler than full pair trading.
"""
...
class PremiumDiscountConfig:
    ...
    min_discount_pct: float = 1.0  # Must be at least 1% discount to trade
    exit_premium_pct: float = 0.5   # Exit when back to 0.5% premium
    max_holding_days: int = 45
    min_observation_days: int = 60  # Need this much history to determine premium
...
class ABPremiumDiscountBacktester:
    ...
    def determine_premium_share(self, data: pd.DataFrame, company: str) -> str:
        ...
        # The share with positive average premium is the premium share
    ...
    def generate_signals(self, data: pd.DataFrame, company: str) -> pd.DataFrame:
        ...
        for _, row in data.iterrows():
            ...
            if premium_share == 'A':
                # A is usually premium, buy A when it's at discount to B
                if a_premium <= -self.config.min_discount_pct:  # A trading at discount
                    action = PremiumAction.BUY_A
                else:
                    action = PremiumAction.HOLD
            else:
                # B is usually premium, buy B when it's at discount to A  
                if b_premium <= -self.config.min_discount_pct:  # B trading at discount
    ...
    async def run_backtest(self) -> Dict:
        ...
        print(f"🚀 Starting A/B Premium-Discount Arbitrage Backtest")
        print(f"   📅 Period: {self.config.start_date.date()} to {self.config.end_date.date()}")
        print(f"   💰 Capital: ${self.config.initial_capital:,.0f}")
        print(f"   📊 Strategy: Buy premium share when it trades at discount")
        ...
        for company in companies:
            ...
            if len(data) > self.config.min_observation_days:
                all_data[company] = self.generate_signals(data, company)
                premium_shares[company] = self.premium_share_cache.get(company, 'A')
        
        if not all_data:
            logger.error("❌ No price data found")
            return {}
            
        print(f"📊 Premium share identification:")
            ...
            for company, pending_trade in self.pending_orders.items():
                if company in all_data:
                    ...
                    if not current_data.empty:
                        ...
                        # Execute immediately (premium-discount is less time sensitive)
    ...
    def print_results(self, results: Dict):
        ...
        print(f"\n" + "="*80)
        print(f"🎯 A/B PREMIUM-DISCOUNT ARBITRAGE BACKTEST RESULTS")
        print(f"="*80)
...
async def main():
    ...
    config = PremiumDiscountConfig(
        ...
        lookback_window=120,
        min_discount_pct=1.0,  # Must be 1%+ discount
        exit_premium_pct=0.5,  # Exit at 0.5% premium
        max_holding_days=45
    )
