#!/usr/bin/env python3
"""
Fixed Scaled Document Anomaly Strategy Backtest.
...
class ScaledDocumentBacktester:
    ...
    async def discover_tradeable_symbols(self, start_date: date, end_date: date) -> List[str]:
        """Discover all symbols with sufficient market data in the backtest period."""
        print(f"\n🔍 Discovering symbols with data from {start_date} to {end_date}...")
        
        # First check what data we have
        data_check = await self.db_conn.fetchrow("""
        ...
        print(f"   Database has {data_check['symbol_count']} symbols, "
              f"{data_check['total_records']:,} records")
        print(f"   Date range: {data_check['min_date']} to {data_check['max_date']}")
        
        # Get symbols with good data coverage in our backtest period
        symbols_query = """
            SELECT symbol, 
                   COUNT(*) as days,
        ...
        # Require at least 60% of the trading days
        min_days_required = int((end_date - start_date).days * 0.6)
        
        symbol_rows = await self.db_conn.fetch(
            symbols_query, start_date, end_date, min_days_required
        )
        
        print(f"   Found {len(symbol_rows)} symbols with {min_days_required}+ days in period")
        
        # Filter to reasonable universe size and show details
        tradeable_symbols = []
        for i, row in enumerate(symbol_rows[:30]):  # Top 30 by data quality
            symbol = row['symbol']
            days = row['days']
            ...
            if i < 15:  # Show first 15
                print(f"     {symbol:<10}: {days:>3} days, avg volume {avg_vol:>12,.0f}")
        
        # If still no symbols, try with more relaxed criteria
        if not tradeable_symbols:
            ...
            relaxed_symbols = await self.db_conn.fetch(relaxed_query, start_date)
            tradeable_symbols = [row['symbol'] for row in relaxed_symbols]
            print(f"   Found {len(tradeable_symbols)} symbols with relaxed criteria")
        
        return tradeable_symbols[:15]  # Return top 15
    ...
    async def run_scaled_backtest(
        ...
        # Discover tradeable symbols for our specific period
        all_symbols = await self.discover_tradeable_symbols(start_date, end_date)
        
        if not all_symbols:
            print("\n❌ No symbols found with sufficient data")
            return {}
        ...
        # Load market data with buffer for indicators
        buffer_start = start_date - timedelta(days=60)
        print(f"\n📊 Loading market data (with 60-day buffer for indicators)...")
        market_data_dict = {}
        
        for i, symbol in enumerate(symbols):
            market_data = await self.get_market_data_for_symbol(symbol, buffer_start, end_date)
...
async def main():
    ...
    try:
        ...
        # Use the date range we know we have data for
        # Based on previous conversation, we ingested data up to Nov 2024
        end_date = date(2024, 10, 31)  # End of October 2024
        start_date = date(2024, 5, 1)   # 6 months back
