#!/usr/bin/env python3
"""
Fixed 787 companies ingestion that works with existing table structure.
No table modifications - just comprehensive error tracking.
"""
...
async def store_price_data_safe(conn, company_info: dict, yahoo_symbol: str, get_max_history: bool = True) -> Dict[str, any]:
    """Safely store price data with existing table structure - gets ALL available historical data"""
    ...
    try:
        # Fetch from Yahoo - GET ALL AVAILABLE HISTORICAL DATA
        ticker = yf.Ticker(yahoo_symbol)
        if get_max_history:
            hist = ticker.history(period="max", auto_adjust=False)  # Get ALL available data
        else:
            # Fallback to last 5 years if max fails
        ...
        # Store price data using existing table structure
        insert_count = 0
        first_error = None
        ...
        for date_idx, row in hist.iterrows():
            ...
            try:
                # Use the existing table structure - symbol is required
                await conn.execute("""
                ...
                """, 
                    company_info['primary_ticker'],  # symbol is required and must not be null
                    date_idx.date(),
...
async def ingest_787_companies_fixed():
    """Fixed ingestion that works with existing table structure"""
    ...
    try:
        ...
        companies = await conn.fetch("""
            SELECT 
                ...
                country
            FROM company_master
            WHERE primary_ticker IS NOT NULL  -- Only companies with tickers
            ORDER BY 
        ...
        print(f"🚀 Starting FIXED ingestion for {len(companies)} companies")
        print("📊 Working with existing table structure - no modifications")
        print("=" * 80)
        ...
        for i, company in enumerate(companies, 1):
            ...
            # Check existing data (for information only - we'll get ALL available data)
            ...
            if validation['validation_status'] == 'good_data':
                ...
                # Attempt to store ALL available historical data
            ...
            if i % 25 == 0:
                ...
                print(f"   🏃‍♂️ Rate: {rate:.1f} companies/min")
        ...
        # Print summary
        print(f"\n" + "=" * 80)
        print(f"🎉 FIXED 787 COMPANIES INGESTION COMPLETE!")
        print(f"=" * 80)
        ...
        if len(tracker.results['success']) > 0:
            ...
            for company in tracker.results['success'][:5]:
                print(f"   ✅ {company['company_name'][:30]:30} -> {company['years_of_data']:.1f} years")
        
        print(f"\n💾 Comprehensive results saved to: {results_file}")
        print(f"\n🔧 Next steps:")
        print(f"   1. Review failed companies: python fix_failed_tickers.py")
        print(f"   2. Apply ticker fixes and re-run ingestion")
        print(f"   3. Validate results against document temporal anomalies")
...
if __name__ == "__main__":
    print("🌟 FIXED 787 COMPANIES INGESTION")
    print("Works with existing table structure + comprehensive error tracking")
    print("=" * 80)
    
    asyncio.run(ingest_787_companies_fixed())